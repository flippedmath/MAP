/**
 * Temporary teacher batch page: all questions needing manual grading,
 * grouped by question number then student. Saves via per-attempt save URLs.
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  const cfg = window.GRADE_MANUAL_BATCH_CONFIG || {};
  ...
  const questionSwitch = document.getElementById('grades-question-switch');

  if (questionSwitch && cfg.questionSwitchUrlTemplate) {
    questionSwitch.addEventListener('change', () => {
      const slot = String(questionSwitch.value || '').trim();
      if (!slot) return;
      window.location.href = String(cfg.questionSwitchUrlTemplate).replace(
        '__SLOT__',
  ...
  function saveUrlForAttempt(attemptId) {
    const template = String(cfg.saveUrlTemplate || '');
    return template.replace('__ATTEMPT__', String(attemptId));
  }

  function studentAsideHtml(student) {
    const s = student || {};
    const rows = [
      ['Username', s.username || '—'],
    ...
    return `<dl class="manual-batch-student">${rows.map(([k, v]) =>
  ...
  try {
    setStatus('Loading manual questions…');
    const res = await fetch(cfg.payloadUrl, {
      headers: { Accept: 'application/json' },
    });
    const data = await res.json();
    if (!res.ok || !data.success) {
      throw new Error(data.error || 'Failed to load manual queue.');
    }
    ...
    items.forEach((item, itemIdx) => {
      ...
      if (!cfg.hideGroupTitles && slot !== lastSlot) {
        const heading = document.createElement('h3');
        heading.className = 'manual-batch-group-title';
        heading.textContent = `Question ${slot}`;
        problemsEl.appendChild(heading);
        lastSlot = slot;
      } else {
        lastSlot = slot;
      }
      ...
      wrap.className = 'manual-batch-item';
      wrap.dataset.attemptId = String(attemptId);
      wrap.innerHTML = studentAsideHtml(item.student);
      problemsEl.appendChild(wrap);

      const card = document.createElement('article');
      card.className = 'practice-problem-card manual-batch-question';
      card.dataset.problemRowId = String(problem.problem_row_id);
      card.dataset.attemptId = String(attemptId);

      const manualBadge = problem.requires_manual_grading
        ? '<span class="manual-pill" data-role="manual-pill">Needs manual grade</span>'
        : '';
      const acceptZeroBtn = problem.requires_manual_grading
      ...
      card.innerHTML = `
        <div class="practice-problem-meta">
          <h3 class="practice-problem-title">${escapeHtml(slot)}. ${escapeHtml(problem.title || `Question ${slot}`)}</h3>
          ${manualBadge}
      ...
      api.renderPreview(previewTarget, problem.body_html || '<p><br></p>', {
        ...
        previewNamePrefix: `batch${attemptId}q${slot}i${itemIdx}`,
      });
      ...
      (problem.fields || []).forEach((field, fieldIdx) => {
        ...
        if (hasVisual) {
          const previewId = `batch-p${problem.problem_row_id}-a${attemptId}-f${fieldIdx}-${String(field.field_token || fieldIdx)}`;
    ...
    setStatus(
      items.length
        ? `${items.length} question${items.length === 1 ? '' : 's'} needing manual grading`
        : 'No questions currently need manual grading.'
    );
    ...
    function collectFieldUpdate(input) {
      ...
      const attemptId = Number(earnedInput.dataset.attemptId);
      if (!Number.isFinite(problemRowId) || !fieldToken || !Number.isFinite(attemptId)) return null;
    ...
    async function postScoreUpdates(attemptId, updates) {
      const token = csrfToken();
      if (!token) {
        throw new Error('Missing CSRF token — refresh the page and try again.');
      }
      const saveRes = await fetch(saveUrlForAttempt(attemptId), {
        method: 'POST',
    ...
    async function saveField(input) {
      ...
      saving = true;
      try {
        await postScoreUpdates(collected.attemptId, [collected.update]);
    ...
    problemsEl.querySelectorAll('[data-role="accept-zero"]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        if (!card) return;
        const attemptId = Number(card.dataset.attemptId);
        if (!Number.isFinite(attemptId)) return;
        const manualRows = Array.from(card.querySelectorAll('.grade-field-row.manual-flag'));
        ...
        btn.disabled = true;
        try {
          await postScoreUpdates(attemptId, updates);
          rows.forEach((row) => {
            showSavedNote(row);
            markFieldGraded(row);
          });
    ...
    console.error(err);
    setStatus(err.message || 'Failed to load manual queue.');
  }
});
